...
async function ghReadFile(cfg) {
  ...
  fileSha = j.sha || null;
  return j.content ? b64decode(j.content) : "";
}
async function ghWriteFile(cfg, text) {
  const body = {
    message: `SalesDesk update ${(/* @__PURE__ */ new Date()).toISOString()}`,
  ...
  if (res.status === 401) throw new Error("Token rejected (401)");
  if (res.status === 403) throw new Error("Token lacks write access (403)");
...
function SalesDesk() {
  ...
  const loadFromCloud = async (c) => {
    ...
    try {
      const text = await ghReadFile(c);
      const dirty = await idbGet(DIRTY_KEY).catch(() => false);
      if (dirty && cached) {
        setPhase("ready");
        try {
          await ghWriteFile(c, cached);
  ...
  const refresh = async () => {
    setSaveState("saving");
    try {
      const text = await ghReadFile(cfg);
      skipSave.current = true;
      setDb(text ? hydrate(text) : EMPTY);
      try {
        await idbSet(CACHE_KEY, text || JSON.stringify(EMPTY));
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { fontFamily: FONT_BODY, background: C.bg, minHeight: "100vh", color: C.ink } }, /* @__PURE__ */ React.createElement("style", null, fontCss), /* @__PURE__ */ React.createElement("div", { className: "sd-shell", style: { display: "flex", minHeight: "100vh", maxWidth: 1180, margin: "0 auto" } }, /* @__PURE__ */ React.createElement("aside", { className: "sd-rail", style: {
    ...
    "div",
    {
      ...
      title: `${cfg.owner || ""}/${cfg.repo || ""} \xB7 ${fileName}`
    },
    "\u2601 ",
    cfg.repo || "repo",
    "/",
    fileName
  ), /* @__PURE__ */ React.createElement("div", { className: "sd-footer", style: { display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(Btn, { kind: "ghost", small: true, onClick: refresh, title: "Pull the latest from GitHub" }, "Refresh"), /* @__PURE__ */ React.createElement(Btn, { kind: "ghost", small: true, onClick: exportJson, title: "Download a backup copy" }, "Export"), isOwner && /* @__PURE__ */ React.createElement(Btn, { kind: "ghost", small: true, onClick: signOut, title: "Disconnect this device" }, "Sign out"))), /* @__PURE__ */ React.createElement("main", { className: "sd-main", style: { flex: 1, padding: "26px 34px 64px", minWidth: 0 } }, !isOwner && /* @__PURE__ */ React.createElement(
    TodayView,
    {
      db,
      accById,
...
function ConfigScreen({ cfg, err, fontCss, onSave }) {
  ...
  const [repo, setRepo] = useState(cfg.repo || "salesdesk-data");
